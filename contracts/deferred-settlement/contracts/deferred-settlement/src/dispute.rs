use crate::types::{DataKey, DeferredTransaction, TransactionStatus, TransactionEvent, Error};
...
pub fn initiate_dispute(env: Env, caller: Address, transaction_id: u128) -> Result<(), Error> {
    ...
    env.events().publish(("DeferredSettlement", Symbol::new(&env, "dispute_initiated")),
        TransactionEvent::DisputeInitiated(transaction_id));

    Ok(())
}

pub fn resolve_dispute(env: Env, caller: Address, transaction_id: u128, release_to_seller: bool) -> Result<(), Error> {
    caller.require_auth();
    ...
    env.events().publish(("DeferredSettlement", Symbol::new(&env, "dispute_resolved")),
